...
// include Function  file
    ...
    else if($usertype == "5")
    {
      ...
      $_SESSION['schooldiaryid'] = $schooldiaryid;
      $getallschooldiary = $axisclass->getAllInfoWhere('schooldiary',$schooldiaryid);
     
      while ($schooldiary = mysqli_fetch_array($getallschooldiary)){
            $title = $schooldiary['title'];
            $coverimage_path = $schooldiary['coverimage_path'];
            $pdf_path = $schooldiary['pdf_path'];
            $is_private = $schooldiary['is_private'];
            $is_classworkd_or_homework = $schooldiary['is_classworkd_or_homework'];
            $description = $schooldiary['description'];
            $subject_id = $schooldiary['subject_id'];
      }
    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header bg-primary text-white">
          <h5 class="card-title">Update School diary</h5>
        </div>
        <div class="card-body">
      <form id="schooldiaryupdate">
   <div class="loader">
      <!-- Add your loader GIF here -->
      <img src="../assets/images/loader.gif" alt="Loader">
    </div>
    <p class="text-center text-medium-emphasis">Update School diary</p>
    ...
    <div class="col-md-12">
        <div class="form-group">
            ...
            <select name="is_classworkd_or_homework" id="is_classworkd_or_homework" class="form-control">
                <option value="">Please Select</option>
                <option value="0" <?php if($is_classworkd_or_homework=="0"){echo "selected";} ?>>class-work</option>
                <option value="1" <?php if($is_classworkd_or_homework=="1"){echo "selected";} ?>>home-work</option>
            </select>
...
$(document).ready(function() {
    ...
    $('#schooldiaryupdate').submit(function(event) {
        ...
        // Make AJAX request
        $.ajax({
            url: 'update_diary.php', // Replace with the actual URL for updating library data
            type: 'POST',
            data: formData,
            dataType: 'json',
            cache: false,
  ...
  if (parseInt(response) === 1) {
    ...
    window.location.href = 'school-diary.php';
  } else {
