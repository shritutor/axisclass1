...
$timelineQuery = "SELECT t.coverimage_path, t.question, '', t.created_at, u.first_name, u.last_name, u.profile_image, 'questionandanswer' AS record_type
                FROM questionandanswer t
                INNER JOIN user u ON t.user_id = u.id
                WHERE u.divisions_id = '$divisions_id' AND u.classes_id = '$classes_id' AND u.schools_id = '$schools_id' AND t.is_private = 0";


// Combine the queries using UNION ALL
$combinedQuery = "($timelineQuery)
...
foreach ($combinedRecords as $record) {
    if ($count == 1) {
        ...
        $html .= '</div>';
        $html .= '<p class="card-text">' . $record['question'] . '</p>';
        ...
        // $html .= '</div>';
        $html .= '<div class="col-md-4">';
        $html .= '<i class="icon comment-icon far fa-comment"></i> Give Answer';
        $html .= '</div>';
        $html .= '<div class="col-md-4">';
