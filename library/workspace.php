    ...
    <div class="card timeline-header-css">
      <div class="PostHeader row mt-3">
        <div class="text-center">
          <div class="page-header-size">Library<a class="btn btn-primary d-inline textbook-add-button-css" href="addlibrary.php" style="margin-bottom: 8px;" fdprocessedid="fplcra">Add</a></div>
        </div>
      </div>
      <div class="row mb-1">
      ...
      <div class="row mt-2 mb-3">
        ...
        <div class="text-center col-10 col-sm-10 col-md-8 col-lg-8 col-xl-8">
          <button class="btn btn-primary rounded-pill groupbutton-active m-1 library-view" type="button" data-view="my-library">My Library</button>
          <button class="btn btn-primary rounded-pill groupbutton m-1 library-view" type="button" data-view="shared-library">Shared Library</button>
        </div>
        <div class="col-1 col-sm-1 col-md-2 col-lg-2 col-xl-2"></div>
      </div>
    </div>
...
$(document).ready(function() {
    ...
    function searchLibraryRecords(searchText) {
        libraryTable.search(searchText).draw();
    }

    // Load all library records
    $.ajax({
        url: 'search_library.php',
        type: 'POST',
        data: { searchText: '' },
        dataType: 'json',
        success: function(response) {
            if (response && response.length > 0) {
                $.each(response, function(index, record) {
                    ...
                    var actions = '<a href="edit_library.php?id=' + record.id + '">Edit</a> | <a href="delete_library.php?id=' + record.id + '" class="delete-link">Delete</a>';
                    
                    libraryTable.row.add({ coverImage: coverImage, title: title, description: description, actions: actions }).draw(false);
                });
            }
        },
    ...
    $('#libraryTable').on('click', '.delete-link', confirmDelete);
});
